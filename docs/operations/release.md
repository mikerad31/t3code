...
- Repository slug: Hardened builds are pinned to `mikerad31/t3code`; build environment variables
  cannot redirect the updater to the official repository. See
  [Hardened Windows build and profile migration](./hardened-windows-release.md).
...
2. Create an explicit App ID for `com.mikerad31.t3code.hardened` and enable Associated Domains.
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.mikerad31.t3code.hardened` and includes
    Associated Domains.
