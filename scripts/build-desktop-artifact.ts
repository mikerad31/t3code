...
const DESKTOP_APP_ID = DESKTOP_DISTRIBUTION.appId;
...
export function resolveGitHubPublishConfig(updateChannel: "latest" | "nightly") {
  return {
    ...
    ...DESKTOP_DISTRIBUTION.updateRepository,
...
export function resolveDesktopProductName(_version: string): string {
  return desktopPackageJson.productName ?? DESKTOP_DISTRIBUTION.productName;
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "T3-Code-Hardened-${version}-${arch}.${ext}",
  ...
  if (!isDesktopPreviewVersion(version)) {
    if (mockUpdates) {
      ...
      buildConfig.publish = [resolveGitHubPublishConfig(updateChannel)];
    }
  }
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    name: DESKTOP_DISTRIBUTION.packageName,
    version: appVersion,
