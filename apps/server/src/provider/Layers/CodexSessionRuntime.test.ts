      ...
      const client = {
        ...
        request: <M extends "thread/start" | "thread/resume">(
          ...
          return Effect.fail(
            new CodexErrors.CodexAppServerRequestError({
              code: -32603,
              errorMessage: "unexpected typed request",
            }),
          );
        },
      };
    ...
    Effect.gen(function* () {
      ...
      const client = {
        raw: {
          request: (method: string, _payload?: unknown) => {
            calls.push(method as "thread/resume");
            return Effect.succeed(makeThreadOpenResponse("wrong-native-thread"));
          },
        },
        request: <M extends "thread/start" | "thread/resume">(
          method: M,
    ...
    Effect.gen(function* () {
      ...
      const client = {
        raw: {
          request: (method: string, _payload?: unknown) => {
            calls.push(method as "thread/resume");
            return Effect.succeed(makeThreadOpenResponse("requested-native-thread"));
          },
        },
        request: <M extends "thread/start" | "thread/resume">(
          method: M,
